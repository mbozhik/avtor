...
function ButtonGroupText({
  ...
  return <Comp className={cn("bg-gray-medium flex items-center gap-2 rounded-md border border-gray-medium px-4 text-sm font-medium shadow-xs [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4", className)} {...props} />
}

function ButtonGroupSeparator({className, orientation = 'vertical', ...props}: React.ComponentProps<typeof Separator>) {
  return <Separator data-slot="button-group-separator" orientation={orientation} className={cn('bg-gray-medium relative m-0! self-stretch data-[orientation=vertical]:h-auto', className)} {...props} />
}
